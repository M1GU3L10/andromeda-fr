...
const Orders = () => {
  ...
  const filteredOrders = orders.filter((order) =>
    ...
    getUserName(order.User_Id).toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="right-content w-100">
      ...
      <Modal show={showDetailModal} onHide={handleCloseDetail}>
        ...
        <Modal.Body>
          ...
          <p><strong>Monto Total:</strong> {new Intl.NumberFormat('es-CO', { style: 'currency', currency: 'COP' }).format(detailData.Total_Amount)}</p>
          <p><strong>Estado:</strong> {translateStatus(detailData.Status)}</p>
          <p><strong>Usuario:</strong> {getUserName(detailData.User_Id)}</p>
